...
Currencies can be added in the `admin` environment. For example `assetCode` is `EUR`, `assetScale` is `2`, and you will need to add an amount to `liquidity`. In order to have access to your local `admin`, you need to run `docker exec -it rafiki-frontend npm run invite-user -- youraccountemail@email.com`, where you need to replace `youraccountemail.email.com` with the email address provided at signup.
