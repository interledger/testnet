...
export async function waitForVerificationLinkFromLogs(args: {
  ...
  const linkPattern = /Verify email link is:\s+(https?:\/\/\S+\/auth\/verify\/[a-f0-9]+)/g
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (execError.code === 'ENOENT') {
        throw new Error('docker CLI is required to retrieve local verification links')
      }
...
export async function setupVerifiedUser(args: {
  ...
  await signUpForm.getByLabel('E-mail *', { exact: true }).fill(credentials.email)
  await ss('002-signup-email-filled')
  await signUpForm.getByLabel('Password *', { exact: true }).fill(credentials.password)
  ...
  await loginForm.getByLabel('E-mail *', { exact: true }).fill(credentials.email)
  await ss('010-login-email-filled')
  await loginForm.getByLabel('Password *', { exact: true }).fill(credentials.password)
