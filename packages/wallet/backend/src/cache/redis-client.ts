...
export interface IRedisClient {
  set<T>(key: string, value: T | string, options?: EntryOptions): Promise<string>;
...
export class RedisClient implements IRedisClient {
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    if (serializedValue) {
      return typeof serializedValue === 'string' ? JSON.parse(serializedValue) as T : serializedValue;
    }
    return null;
  }
