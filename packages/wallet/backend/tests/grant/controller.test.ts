...
describe('Grant Controller',()=>{
    ...
    const createMockGrantControllerDeps = (isFailure?: boolean) => {
        const grantControllerDepsMocked = {
            rafikiAuthService: {
                ...
                revokeGrant: isFailure? jest.fn().mockRejectedValueOnce(new Error('Unexpected error')) : jest.fn(),
    ...
    describe('Get list of grants', ()=>{
        it('should return array', async ()=>{
            ...
            expect(res._getJSONData()).toMatchObject({
                success:true, message:'Success', data:['grant1']
            })
        })
    })
    ...
    describe('Get By Id',()=>{
        it('should return a grant', async ()=>{
            ...
            expect(res.statusCode).toBe(200)
            expect(res._getJSONData()).toMatchObject({
                success:true, message:'Success', data:{id:'grant'}
            })
        })
    })
