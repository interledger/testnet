...
export const AppLayout = ({ children }: AppLayoutProps) => {
  ...
  return (
    <>
      ...
      <main className="mt-[84px] px-8 py-7 md:mt-0 md:px-16 md:py-12 min-w-0 md:[grid-column:2/3]">
        {children}
        <Toaster />
        <Bubbles className="fixed inset-y-0 right-0 hidden h-full lg:block pointer-events-none" />
      </main>
    </>
  )
}
